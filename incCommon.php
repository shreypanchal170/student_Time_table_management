	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'schools' => array('Schools', '', 'resources/table_icons/building.png', 'None'),
			'departments' => array('Departments', '', 'resources/table_icons/chart_organisation.png', 'None'),
			'class_time_table' => array('Class time table', '', 'resources/table_icons/blackboard_drawing.png', 'None'),
			'exam_time_table' => array('Exam time table', '', 'resources/table_icons/books.png', 'None'),
			'personal_time_table' => array('Personal time table', '', 'resources/table_icons/clock_.png', 'None'),
			'student_details' => array('Student details', '', 'resources/table_icons/administrator.png', 'None'),
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Jisort_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Jisort_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Jisort_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Jisort_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Jisort</a>
			</div>
