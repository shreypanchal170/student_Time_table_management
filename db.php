<?php
	...
	*/
	...
	function db_select_db($dbname, $link = NULL){
		...
		switch(DATABASE){
			case 'mysql':
				return mysqli_select_db($dbname, $link);
			case 'mysqli':
	...
	function db_field_name($res, $field_offset){
		switch(DATABASE){
			case 'mysql':
				return @mysqli_field_name($res, $field_offset);
			case 'mysqli':
	...
	function db_field_type($res, $field_offset){
		switch(DATABASE){
			case 'mysql':
				return @mysqli_field_type($res, $field_offset);
			case 'mysqli':
				$fo = @mysqli_fetch_field_direct($res, $field_offset);
				if($fo === false) return false;
				return $fo->type;
	...
	function db_escape($str = NULL, $link = NULL){
		...
		switch(DATABASE){
			case 'mysql':
				if(function_exists('mysql_real_escape_string'))    return mysqli_real_escape_string($str, $link);
	...
	function db_connect($host = NULL, $username = NULL, $passwd = NULL, $dbname = NULL, $port = NULL, $socket = NULL){
		switch(DATABASE){
			case 'mysql':
				...
				@mysqli_query("SET NAMES '" . mysql_charset . "'");
				if($dbname){ if(!mysqli_select_db($dbname, $link)) return false; }
				return $link;
			case 'mysqli':
